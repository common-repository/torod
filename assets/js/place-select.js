			...
			$('select.city_select:visible').each(function () {
				var select2_args = $.extend({
					...
					matcher: function (params, data) {
						...
						var dataAr = $(data.element).attr('data-ar');
						var dataEn = $(data.element).attr('data-en');
		...
		if (cities[country]) {
			...
			if (cities[country] instanceof Array) {
				...
				if (cities[country][state]) {
					if (cities[country][state].length > 0) {
	...
	function cityToSelect($citybox, current_cities) {
		...
		for (var index in current_cities) {
			if (current_cities.hasOwnProperty(index)) {
				...
				var defaultlang = wc_city_select_params.dlang;
				if ('SA' == $('#billing_country').val()) {
					var cityNameAr = cityName.ar; // Arapça şehir adını alın
					var cityNameEn = cityName.en; // İngilizce şehir adını alın
					if (defaultlang == 'ar') {
						options = options + '<option value="' + cityNameAr + '" data-ar="' + cityNameAr + '" data-en="' + cityNameEn + '">' + cityNameAr + '</option>';
					} else {
						options = options + '<option value="' + cityNameEn + '" data-ar="' + cityNameAr + '" data-en="' + cityNameEn + '">' + cityNameEn + '</option>';
					}
				} else if ('KW' == $('#billing_country').val()) {
					var cityNameAr = cityName.ar;
					var cityNameEn = cityName.en;
